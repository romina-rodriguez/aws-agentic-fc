"""AI Soccer Forward 2 Agent — Player 4. Nova Pro."""
...
SYSTEM_PROMPT = f"""You control player {MY_PLAYER_ID} (FWD, right) in 5v5 soccer. Return ONLY a JSON array with one command. No text, no thinking, no markdown.

RULES (check in order, do the FIRST that matches):
1. If hasBall=True AND distOppGoal < 45 → SHOOT aim_location="BL", power=0.9
2. If hasBall=True AND player 3 is closer to goal than me AND distOppGoal > 45 → PASS type THROUGH to player 3
